...
const HomePage = () => {
  ...
  const fetchDepartments = async () => {
    ...
    try {
      ...
      const departmentsWithData = response.queryResult.map((dept) => ({
        ...dept,
        tag: dept.tag || generateDepartmentTag(dept.name),
        userCount: dept.userCount || Math.floor(Math.random() * 50) + 5,
        moderator: dept.moderator || {
          id: dept.moderatorId || 0,
          firstName: 'Модератор',
          lastName: dept.name,
          email: `moderator@${dept.name.toLowerCase().replace(/\s+/g, '')}.ru`,
          departmentsIds: [dept.id],
          moderatorId: 0,
          role: 'MODERATOR' as const
        }
      }));
      
      setDepartments(departmentsWithData);
    } catch (err: any) {
      console.error('Error fetching departments:', err);
      setError(err.response?.data?.message || err.message || 'Ошибка при загрузке департаментов');
    } finally {
      setLoading(false);
    }
  };

  // Функция для генерации тегов департаментов
  const generateDepartmentTag = (name: string): string => {
